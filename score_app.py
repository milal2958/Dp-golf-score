...
... 
... # 구글 시트 연결 설정
... # 시트 공유 설정에서 '링크가 있는 모든 사용자 - 편집자'로 되어 있어야 합니다.
... spreadsheet = "https://docs.google.com/spreadsheets/d/17N4UmhNqVmSNo_6Al4iVU18Ds1jPX2brOPcAKfepjDQ/edit?hl=ko&gid=0#gid=0"
... 
... conn = st.connection("gsheets", type=GSheetsConnection)
... 
... # 데이터 불러오기 함수
... def fetch_data():
...     return conn.read(spreadsheet=spreadsheet, usecols=list(range(9)), ttl=0)
... 
... df = fetch_data()
... 
... # --- 1. 입력 섹션 ---
...
...         par = c2.number_input("파", 0, 18, 0)
        ...
        if submit:
            if not course or not name:
                ...
                # 구글 시트 업데이트
                conn.update(spreadsheet=spreadsheet, data=updated_df)
